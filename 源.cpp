...
//  员工Id唯一且不可修改
void infoaddition() {
    // 你这个id库 应该是全局变量吧 你写这里岂不是要调用一次写一遍这个函数 然后都是从1-1000 浪费时间不说 还有问题 导致你的id 并不唯一
    int p[1000];                                                  //生成Id库
        ...
        if (strcmp(employeeinfo[x].Name, "0") == 0) {             //跳过已有数据，从其后的第一个空数组元素开始赋值
            ...
            for (int a = 0; a < 100; a++) {
                if (!p[a] == 0) {
                    ...
                    break;
                // else 后的break 啥意思。。。 
                // a 从0开始 到100 你的id库后边的永远用不着
                // 感觉逻辑有问题 再想想
                // 我的想法不是建立你这种 id库 第一种方法是 有一个全局变量 存最大的 id值 每次更新 注意初始化和更新 第二种是当数组是根据用户id排列的时候
                // 获取最后一个的id 的值 然后加1  你这种id库的好处是 能存所有 能把 删除掉的id 重复利用 但是不知道你是否维护了在删除的时候
            }
            cout << "添加成功！" << endl;
            return;
        }
...
//  通过用户Id查询用户信息
void FindById() {
    //1.输入并获取待查询用户Id   这些注释放里边 给自己注释 划分模块清楚 外边注释有自己的格式
    string p;
    int j;
    cout << "请输入用户Id：";
    cin >> p;
